/*
  # Enhanced Search Feature with Relevance Scoring

  1. Changes
    - Add text search capabilities with relevance scoring
    - Prioritize matches in product names and keywords
    - Improve search performance with GIN indexes
    - Add scoring system for search results
...
-- Create enhanced search function with scoring
...
CREATE INDEX IF NOT EXISTS products_name_tsv_idx ON products USING gin(to_tsvector('english', name));
CREATE INDEX IF NOT EXISTS products_description_tsv_idx ON products USING gin(to_tsvector('english', description));
